...
<style type="text/css">
	...
	<fmt:message bundle="${loc}" key="local.addServicePlanButton.name" var="add_sp_button" />
	<fmt:message bundle="${loc}" key="local.updateServicePlanButton.name" var="update_sp_button" />
	<fmt:message bundle="${loc}" key="local.deleteServicePlanButton.name" var="delete_sp_button" />
	...
	<fmt:message bundle="${loc}" key="local.addPromoButton.name" var="add_promo_button"/>
	<fmt:message bundle="${loc}" key="local.deletePromoButton.name" var="delete_promo_button"/>
	<fmt:message bundle="${loc}" key="local.servicePlanId" var="service_plan_id" />
	...
	<fmt:message bundle="${loc}" key="local.accessDiscount" var="access_discount" />
	<fmt:message bundle="${loc}" key="local.trafficBonus" var="traffic_bonus" />	
	<fmt:message bundle="${loc}" key="local.servicePlanDescription" var="description" />
	<fmt:message bundle="${loc}" key="local.servicePlanCard" var="service_plan_card" />
	...
	<fmt:message bundle="${loc}" key="local.idFillMessage" var="id_fill_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanFillMessage" var="sp_fill_message" />
	<fmt:message bundle="${loc}" key="local.trafficFillMessage" var="traffic_fill_message" />
	<fmt:message bundle="${loc}" key="local.feeFillMessage" var="fee_fill_message" />
	<fmt:message bundle="${loc}" key="local.accessCostFillMessage" var="access_cost_fill_message" />
	<fmt:message bundle="${loc}" key="local.descriptionFillMessage" var="description_fill_message" />	
	<fmt:message bundle="${loc}" key="local.promoFillMessage" var="promo_fill_message" />
	<fmt:message bundle="${loc}" key="local.accessDiscountFillMessage" var="access_discount_fill_message" />
	<fmt:message bundle="${loc}" key="local.trafficBonusFillMessage" var="traffic_bonus_fill_message" />	
	<fmt:message bundle="${loc}" key="local.servicePlanCreationSuccessMessage" var="sp_creation_success_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanUpdateSuccessMessage" var="sp_update_success_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanWasntUpdatedMessage" var="sp_wasnt_updated_message" />
	<fmt:message bundle="${loc}" key="local.promoCreationSuccessMessage" var="promo_creation_success_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanIsUnderPromoMessage" var="sp_under_promo_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanHasNoPromoMessage" var="sp_has_no_promo_message" />
	<fmt:message bundle="${loc}" key="local.promoRemoveSuccessMessage" var="promo_remove_success_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanRemoveSuccessMessage" var="sp_remove_success_message" />
...
<body>
	...
	<form action="FrontController" id="getSpListForm" class="invisible" method="get" >
		...
		<input type="submit" class="btn btn-outline-secondary btn-sm" value="${find_service_plans}" /><br />	
	</form>
	...
	<input type="hidden" name="promoExistsMessage" id="promoExistsMessage" value="${sp_under_promo_message}" />
	<input type="hidden" name="noPromoMessage" id="noPromoMessage" value="${sp_has_no_promo_message}" />

	<button type="button" class="btn btn-outline-secondary btn-sm" id="openSpFormAdd" data-toggle="modal" onclick="prepareSpAddForm()"
		data-target="#spForm">${add_sp_button}</button>
	<button type="button" class="btn btn-outline-secondary btn-sm" id="openSpFormUpdate" data-toggle="modal" onclick="prepareSpUpdateForm(openSpFormUpdate)"
		data-target="#spForm">${update_sp_button}</button>
	<button type="button" class="btn btn-outline-secondary btn-sm" id="openSpFormDelete" data-toggle="modal" onclick="prepareSpDeleteForm(openSpFormDelete)"
		data-target="#spForm">${delete_sp_button}</button><br><br>	
	<button type="button" class="btn btn-outline-secondary btn-sm" id="openPromoFormAdd" data-toggle="modal" onclick="preparePromoAddForm(openPromoFormAdd)"
		data-target="#promoForm">${add_promo_button}</button>
	<button type="button" class="btn btn-outline-secondary btn-sm" id="openPromoFormDelete" data-toggle="modal" onclick="preparePromoDeleteForm(openPromoFormDelete)"
		data-target="#promoForm">${delete_promo_button}</button>	
		
	<form action="FrontController" method="post">
		<input type="hidden" name="spId" id="spId" pattern="[\d]{1,10}" value="0" /> 
		<div class="modal fade" data-backdrop="false" id="spForm" tabindex="-1" role="dialog" aria-labelledby="spFormLabel"
			aria-hidden="true">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-group row">
							...
							<div class="col-sm-4">
								<input type="text" pattern="([A-Za-z]{1})([A-Za-z0-9-_\s]{5,50})"
						...
						<div class="form-group row">
							...
							<div class="col-sm-4">
								<input type="text" pattern="([1-9]{1})([0-9]{0,9})?$"
	...
	<form action="FrontController" method="post">
		<input type="hidden" name="promoSpId" id="promoSpId" pattern="[\d]{1,10}" value="0" /> 
		<input type="hidden" name="promoId" id="promoId" pattern="[\d]{1,10}" value="0" /> 
		<div class="modal fade" data-backdrop="false" id="promoForm" tabindex="-1" role="dialog" aria-labelledby="promoFormLabel"
			aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
					...
					<div class="modal-body">
						...
						<div class="form-group row">
							...
							<div class="col-sm-4">
								<input type="text" pattern="([1-9]{1}[0-9]{0,10})"
		...
		<c:if test="${not empty requestScope.validationErrorList}">
			<c:forEach var="validation_error" items="${requestScope.validationErrorList}">
