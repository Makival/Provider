...
<style type="text/css">
	...
	<fmt:message bundle="${loc}" key="local.refillButton.name" var="refill_button" />
	<fmt:message bundle="${loc}" key="local.executeButton.name" var="execute_button" />
	<fmt:message bundle="${loc}" key="local.changeServicePlanButton.name" var="change_sp_button" />	
	...
	<fmt:message bundle="${loc}" key="local.trafficBalance" var="traffic_balance"/>
	...
	<fmt:message bundle="${loc}" key="local.summFillMessage" var="summ_fill_message" />
	<fmt:message bundle="${loc}" key="local.refillBalanceSuccessMessage" var="refill_balance_success_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanChosenSuccess" var="sp_chosen_success_message" />
	<fmt:message bundle="${loc}" key="local.servicePlanNotFound" var="sp_update_not_found_message" />
	<fmt:message bundle="${loc}" key="local.clientAccountHeader" var="client_account_header" />
	<fmt:message bundle="${loc}" key="local.servicePlanDetailsHeader" var="service_plan_details_header" />
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-sm-3 bg-info div-cell">${traffic_balance}</div>
				<div class="col-sm-2">
					<button type="button" id="refillShowButton" class="btn btn-outline-secondary">${refill_button}</button>
				</div>
			</div>
		<div class="row">
		...
		<form action="FrontController" id="refillBalanceForm" method="post" style="display:none">
			<div class="form-group row">
				<label for="surname" class="col-sm-3 col-form-label">${refill_summ}</label>
				<div class="col-sm-2">
					<input type="text" pattern="([1-9]{1}[0-9]{0,7})(\.[0-9]{2})?$"
					oninvalid="this.setCustomValidity('${summ_fill_message}')"
					oninput="setCustomValidity('')"
					class="form-control" name="refillSumm" id="refillSumm" value="" required />
				</div>
				<input type="hidden" name="command" value="refill_balance" /> 
				<input type="submit" class="btn btn-outline-secondary btn-sm" value="${execute_button}" /><br />	
			</div>
		</form>
		...
		<br><p>${service_plan_details_header}</p>
		<div class="row">
		...
		<form action="FrontController" method="get">
			...
			<input type="submit" class="btn btn-outline-secondary btn-sm" value="${change_sp_button}" /><br />	
		</form>
