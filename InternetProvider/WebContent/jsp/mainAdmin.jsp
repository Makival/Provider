...
<fmt:message bundle="${loc}" key="local.registrationButton.name"
	var="reg_button" />
<fmt:message bundle="${loc}" key="local.findUsersButton.name"
	var="find_users_button" />
<fmt:message bundle="${loc}" key="local.userBlockButton.name"
	var="user_block_button" />
<fmt:message bundle="${loc}" key="local.goToServicePlanListButton.name"
	var="go_to_sp_button" />
...
<fmt:message bundle="${loc}" key="local.confirmUserBlockHeader"
	var="confirm_user_block_header" />
<fmt:message bundle="${loc}" key="local.userCreationSuccessMessage"
	var="user_creation_success_message" />
<fmt:message bundle="${loc}" key="local.userUpdateSuccessMessage"
	var="user_update_success_message" />
<fmt:message bundle="${loc}" key="local.userBlockSuccessMessage"
	var="user_block_success_message" />
<fmt:message bundle="${loc}" key="local.chooseUserMessage"
	var="choose_user_message" />
...
<body>
	...
	<div class="container">
		<form action="FrontController" id="getUserListForm" method="get">
			<input type="hidden" name="minUserId"
				value="${requestScope.minUserId}" /> <input type="hidden"
				name="maxUserId" id="maxUserId" value="${requestScope.maxUserId}" />
			<input type="hidden" name="lastUserId" id="lastUserId"
				value="${requestScope.lastUserId}" /> <input type="hidden"
				name="command" value="find_users" /> <input type="hidden"
				name="direction" id="UserListDirection" value="" /> <input
				type="hidden" name="pageNumber" id="pageNumber"
				value="${requestScope.pageNumber}" /> <input type="submit"
				class="btn btn-outline-secondary btn-sm"
				value="${find_users_button}" />
		</form>
		<form action="FrontController" method="post">
			<input type="hidden" name="command" value="find_service_plans" /> <input
				type="submit" class="btn btn-outline-secondary btn-sm"
				value="${go_to_sp_button}" /><br />
		</form>
		...
		<c:if test="${requestScope.userListVisibility eq 'TRUE'}">
			...
			<input type="hidden" name="chooseUserMessage" id="chooseUserMessage"
				value="${choose_user_message}" />

			<form action="FrontController" method="post">
				<input type="hidden" name="chosenUserId" id="chosenUserId"
					pattern="[\d]{1,10}" value="0" />
				<div class="modal fade" data-backdrop="false" id="userForm"
					tabindex="-1" role="dialog" aria-labelledby="userFormLabel"
					aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
							...
							<div class="modal-footer">
								...
								<input type="hidden" name="command" value="user_block" /> <input
									type="submit" class="btn btn-outline-secondary btn-sm"
			...
			<button type="button" class="btn btn-outline-secondary btn-sm"
				id="openUserBlockForm" data-toggle="modal"
				onclick="checkUserId(openUserBlockForm)" data-target="#userForm">${user_block_button}</button>

			<form action="FrontController" method="post">
				<input type="hidden" name="command" value="go_to_reg_page" /> <input
					type="submit" class="btn btn-outline-secondary btn-sm"
					value="${reg_button}" /><br />
			</form>
